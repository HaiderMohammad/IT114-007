...
public class ServerThread extends Thread {
	...
	public void saveMutedUsers() {
		try (FileWriter f = new FileWriter(getClientName() + "mutedUsers.txt")) {
			for(String m : mutedUsers) {
				f.write(m + ",");
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void loadMutedUsers() {
		Path fileName = Path.of(getClientName() + "mutedUsers.txt");
		try {
			String file = Files.readString(fileName);
			String[] userList = file.split(",");
